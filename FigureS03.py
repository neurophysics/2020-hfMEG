...
marker = marker[
    ...
    (marker+trial_win[1])<len(stim_data_sigma)
    ], 0)
        ]
...
# in the hp trials (30-200 Hz), pick the largest peak in the stim_win
stim_hp = meet.epochEEG(stim_data_hp, marker, stim_win)

N20_mean_lat = np.argmax(stim_hp.mean(-1))/srate*1000 + stim_win_ms[0]
N20_win_ms = [N20_mean_lat - 3, N20_mean_lat + 3]
N20_win = np.round(np.array(N20_win_ms)/1000.*srate
        ).astype(int)

N20_hp = meet.epochEEG(stim_data_hp, marker, N20_win)

N20_lat = np.argmax(N20_hp, axis=0)/srate*1000 + N20_win_ms[0]

stim_order = np.argsort(N20_lat[inlier_mask])

st_percentile_steps = np.array([0.1, 0.9]) # for single-trial plotting
avg_percentile_steps = np.array([0.1,0.3, 0.5, 0.7, 0.9]) # for avg plotting
...
# calculate, whether the average between the 20th and the 80th percentile 
# are different
...
avg_gs = mpl.gridspec.GridSpecFromSubplotSpec(subplot_spec=gs[1], nrows=3, ncols=1,
        height_ratios=[1-2*q, q, q])
...
hf_st_ax1.text(0.05, 0.95, r'\textbf{10th} percentile', ha='left', va='top',
        size=7, transform=hf_st_ax1.transAxes)
hf_st_ax2.text(0.05, 0.95, r'\textbf{90th} percentile', ha='left', va='top',
        size=7, transform=hf_st_ax2.transAxes)

#create some invisible dummy lines for the legend
...
lg_st = fig.legend(
        ...
        r'selected according to latency of N20m',
...
fig.savefig(path.join(results_folder, 'FigureS03_{}.pdf'.format(subject)))
fig.savefig(path.join(results_folder, 'FigureS03_{}.png'.format(subject)))
